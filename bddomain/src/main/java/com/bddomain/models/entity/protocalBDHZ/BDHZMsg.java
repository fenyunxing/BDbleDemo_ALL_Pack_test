...
public class BDHZMsg implements CheckImpl {
    ...
    public BDHZMsg(byte[] parambytes) {
        batterydata = new String(parambytes);
        Log.v("FDBDChatTest", "BDHZMsg ===>" + batterydata);
        IfVaild = BDMethod.CheckCKS(batterydata);
        if (IfVaild) {
            String[] items = batterydata.split(",");
            if (items.length > 3) {
     ...
     * 获取北斗盒子专用指令类型
     *
     * @return 北斗盒子专用指令类型 "DLFK" ：电量反馈 "SOS"：紧急求救  "SET"：设置  "CHECK"：检验
     */
